...
#include "saps_base/vl53l8cx_buffers.h"
...
uint8_t VL53L8CX_FIRMWARE_DOWNLOAD_STAGES(VL53L8CX_Configuration *p) { (void)p; return 0; }
uint8_t VL53L8CX_OrdersMCU_Reset(VL53L8CX_Configuration *p) { (void)p; return 0; }
        ...
        VL53L8CX_Configuration      *p_dev)
{
    ...
    printf("\n[ToF Debug] =====================================\n");
    printf("[ToF Debug] VL53L8CX C-Driver Init 시퀀스 진입\n");
    ...
    /* [관문 1] SW reboot sequence */
    printf("[ToF Debug] [관문 1] 소프트웨어 리부팅 시퀀스 전송 중...\n");
    status |= VL53L8CX_WrByte(&(p_dev->platform), 0x7fff, 0x00);
	printf("[ToF Debug]  %d\n", status);
    ...
    printf("[ToF Debug] [관문 1 결과] status 비트 플래그 값: %d\n", status);
    if(status != (uint8_t)0){
        printf("[ToF Debug] ❌ [관문 1]에서 튕겼습니다. 초기 리부팅 패킷에 칩이 응답 안 함.\n");
        goto exit;
    }
    printf("[ToF Debug]     -> [관문 1] 성공! 센서 기본 부팅 확인.\n");

    /* [관문 2] Enable FW access */
    printf("[ToF Debug] [관문 2] 펌웨어(FW) 액세스 권한 활성화 중...\n");
    ...
    printf("[ToF Debug] [관문 2 결과] status 비트 플래그 값: %d\n", status);
    if(status != (uint8_t)0){
        printf("[ToF Debug] ❌ [관문 2]에서 튕겼습니다. 펌웨어 핸드셰이크가 안 됩니다.\n");
        goto exit;
    }
    printf("[ToF Debug]     -> [관문 2] 성공! FW 접근 모드 진입.\n");

    /* [관문 3] Wake up MCU & Download FW */
    printf("[ToF Debug] [관문 3] 센서 내부 MCU 깨우기 및 대용량 FW 다운로드 다운로드 시작...\n");
    status |= VL53L8CX_WrByte(&(p_dev->platform), 0x7fff, 0x00);
    ...
    // 3분할 펌웨어 데이터 주입
    status |= VL53L8CX_FIRMWARE_DOWNLOAD_STAGES(p_dev); // 편의상 이 구간 압축 매핑 체크
    ...
    status |= VL53L8CX_WrMulti(&(p_dev->platform),0, (uint8_t*)&VL53L8CX_FIRMWARE[0x10000],0x5000);
    ...
    status |= VL53L8CX_OrdersMCU_Reset(p_dev); // MCU 리셋 유도 연사
    status |= VL53L8CX_WrByte(&(p_dev->platform), 0x7FFF, 0x00);
    ...
    printf("[ToF Debug] [관문 3 결과] status 비트 플래그 값: %d\n", status);
    if(status != (uint8_t)0){
        printf("[ToF Debug] ❌ [관문 3]에서 튕겼습니다. 대용량 펌웨어 다운로드 직후 MCU 부팅 폴링 실패.\n");
        goto exit;
    }
    printf("[ToF Debug]     -> [관문 3] 성공! 내부 펌웨어 적재 및 MCU 구동 확인.\n");

    /* [관문 4] Firmware checksum 검증 */
    printf("[ToF Debug] [관문 4] 다운로드된 내장 펌웨어 무결성 CRC Checksum 검사 중...\n");
    status |= VL53L8CX_WrByte(&(p_dev->platform), 0x7fff, 0x02);
    ...
    printf("[ToF Debug] 읽어온 칩셋 Checksum: 0x%08x (기대값: 0x0c0b6c9e)\n", crc_checksum);
    if (crc_checksum != (uint32_t)0xc0b6c9e)
    {
        printf("[ToF Debug] ❌ [관문 4] Checksum 불일치 오류 발생! 데이터가 깨져서 전송됨.\n");
        status |= VL53L8CX_STATUS_FW_CHECKSUM_FAIL;
        goto exit;
    }
    printf("[ToF Debug]     -> [관문 4] 성공! Checksum 패스.\n");

    /* [관문 5] NVM Calibration Data 로드 및 최종 셋업 */
    printf("[ToF Debug] [관문 5] 보정(NVM) 및 기본 파라미터 영역 구성 설정 구성 중...\n");
    status |= VL53L8CX_WrMulti(&(p_dev->platform), 0x2fd8, (uint8_t*)VL53L8CX_GET_NVM_CMD, sizeof(VL53L8CX_GET_NVM_CMD));
    status |= _vl53l8cx_poll_for_answer(p_dev, 4, 0, VL53L8CX_UI_CMD_STATUS, 0xff, 2);
    status |= VL53L8CX_RdMulti(&(p_dev->platform), VL53L8CX_UI_CMD_START, p_dev->temp_buffer, VL53L8CX_NVM_DATA_SIZE);
    memcpy(p_dev->offset_data, p_dev->temp_buffer, VL53L8CX_OFFSET_BUFFER_SIZE);
    status |= _vl53l8cx_send_offset_data(p_dev, VL53L8CX_RESOLUTION_4X4);

    memcpy(p_dev->xtalk_data, (uint8_t*)VL53L8CX_DEFAULT_XTALK, VL53L8CX_XTALK_BUFFER_SIZE);
    status |= _vl53l8cx_send_xtalk_data(p_dev, VL53L8CX_RESOLUTION_4X4);

    status |= VL53L8CX_WrMulti(&(p_dev->platform), 0x2c34, p_dev->default_configuration, sizeof(VL53L8CX_DEFAULT_CONFIGURATION));
    status |= _vl53l8cx_poll_for_answer(p_dev, 4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03);

    status |= vl53l8cx_dci_write_data(p_dev, (uint8_t*)&pipe_ctrl, VL53L8CX_DCI_PIPE_CONTROL, (uint16_t)sizeof(pipe_ctrl));
	#if VL53L8CX_NB_TARGET_PER_ZONE != 1
		tmp = VL53L8CX_NB_TARGET_PER_ZONE;
		status |= vl53l8cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L8CX_DCI_FW_NB_TARGET, 16, (uint8_t*)&tmp, 1, 0x0C);
	#endif

		status |= vl53l8cx_dci_write_data(p_dev, (uint8_t*)&single_range, VL53L8CX_DCI_SINGLE_RANGE, (uint16_t)sizeof(single_range));
		
		printf("[ToF Debug] [관문 5 결과] 최종 구성 status: %d\n", status);

	exit:
		printf("[ToF Debug] 최종 반환 상태코드(status): %d\n", status);
		printf("[ToF Debug] =====================================\n\n");
		return status;
}
